...
- `server.ts` — the backend: a todo store in `bb.storage.kv`, RPC methods
  for the page, a `bb plan-mode` CLI command, a setting, and a realtime signal
...
- `skills/example-todos/SKILL.md` — a skill that tells agents how to keep the list
  with `bb plan-mode`. BB imports it into agent threads automatically.
...
`bb plan-mode add "Ship it"` in a terminal. The page updates at once.
...
```
bb plugin config plan-mode
bb plugin config plan-mode set showDone false
bb plugin reload plan-mode
```
